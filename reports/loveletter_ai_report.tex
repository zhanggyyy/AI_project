...
belief search, expectimax, Monte Carlo tree search, and tabular Q-learning.
...
Q-learning with a heuristic prior consistently outperform random play, while
belief search is harder to tune and has high variance at small sample sizes.
...
The Q-learning agent uses tabular state-action values over compact observation
and action keys. The update rule is
\begin{equation}
...
\end{equation}
The agent is trained by self-play and then evaluated with exploration disabled.
...
agents, each reaching 77.5 percent in this pilot run. Q-learning is close at
75.0 percent. Belief search variants improve over random but do not clearly
beat the simpler rule-based baselines at this sample size.
...
BeliefExpectimax & 27-13-0 & 67.5\% & 52.0--79.9\\
BeliefMCTS & 27-13-0 & 67.5\% & 52.0--79.9\\
Q-learning & 30-10-0 & 75.0\% & 59.8--85.8\\
\bottomrule
\end{tabular}
\end{table}
...
Table~\ref{tab:ablations} summarizes the main ablations. Two patterns are
important. First, more computation does not automatically improve a
hidden-information agent at small sample size: extra samples can increase
variance if the belief model or rollout policy is weak. Second, the oracle
expectimax baseline improves with depth, which suggests that the game engine
and search mechanics are working, but also confirms that perfect information
gives a different kind of advantage.
...
\midrule
BeliefExpectimax samples & samples=1 & 31-9-0 & 77.5\% & 62.5--87.7\\
BeliefExpectimax samples & samples=4 & 29-11-0 & 72.5\% & 57.2--83.9\\
BeliefExpectimax samples & samples=8 & 26-14-0 & 65.0\% & 49.5--77.9\\
BeliefExpectimax depth & depth=1 & 27-13-0 & 67.5\% & 52.0--79.9\\
BeliefExpectimax depth & depth=2 & 27-13-0 & 67.5\% & 52.0--79.9\\
BeliefMCTS simulations & simulations=4 & 25-15-0 & 62.5\% & 47.0--75.8\\
BeliefMCTS simulations & simulations=12 & 23-17-0 & 57.5\% & 42.2--71.5\\
BeliefMCTS simulations & simulations=24 & 29-11-0 & 72.5\% & 57.2--83.9\\
Q-learning episodes & episodes=0 & 32-8-0 & 80.0\% & 65.2--89.5\\
Q-learning episodes & episodes=100 & 26-14-0 & 65.0\% & 49.5--77.9\\
Q-learning episodes & episodes=500 & 29-11-0 & 72.5\% & 57.2--83.9\\
...
The Q-learning result also needs careful interpretation. The episode=0
condition still uses the heuristic prior, so it is not a pure untrained random
agent. Self-play training can help, but sparse terminal reward and a compact
tabular key make learning unstable. A higher-scoring extension would report a
learning curve over many seeds and separate the effect of the heuristic prior
from the effect of learned Q-values.
...
\begin{itemize}
    ...
    \item \textbf{RL maturity.} Tabular Q-learning is a useful baseline but
    does not yet demonstrate a clean learning curve.
