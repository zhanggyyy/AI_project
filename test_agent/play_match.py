...
def build_agent(kind: str, seat_name: str):
    ...
    if kind == "greedy":
        return GreedyAgent(f"Greedy-{seat_name}")
    if kind == "improved":
        return ImprovedHeuristicAgent(f"Improved-{seat_name}")
    if kind == "expectimax":
    ...
    if kind == "belief_expectimax":
        depth = prompt_int(f"{seat_name} Belief Expectimax depth", default=2)
        samples = prompt_int(f"{seat_name} Belief samples", default=20)
        return BeliefExpectimaxAgent(
            f"BeliefExpectimax-{seat_name}",
            depth=depth,
            samples=samples,
            debug=False,
        )
    if kind == "belief_mcts":
        simulations = prompt_int(f"{seat_name} Belief MCTS simulations", default=120)
        return BeliefMCTSAgent(
            f"BeliefMCTS-{seat_name}",
